...
export class TestHelpers {
  ...
  static async checkAccessibility(page: Page) {
    ...
    const inputs = await page
      .locator('input[type="text"], input[type="number"], input[type="email"]')
      .all();
   ...
   */
  static async retry<T>(action: () => Promise<T>, maxAttempts = 3, baseDelay = 1000): Promise<T> {
    let lastError: Error;
